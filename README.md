...
- 📖 [Примеры запуска](#-примеры-запуска)
  ...
  - [Hermes + llama.cpp](#hermes--llamacpp)
...
Создание и добавление Telegram бота в Hermes подробно описана на странице [документации](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/telegram)  
Как минимум необходимо создать бота, добавить его токен в переменную `TELEGRAM_BOT_TOKEN` в файл `.env` и добавить ID юзера который может писать боту в переменную `TELEGRAM_ALLOWED_USERS`  
Другие параметры описаны в файле `.env` - там же ссылка на документацию по переменным окружения  

Подключение Telegram появится в `СЕССИИ` и в `CHANNELS` интерфейса Hermes - там же можно сконфигурировать настройки вручную или даже создать бота

Пример настройки бота, который читает все сообщения группы (ID группы -1001234567890), но отвечает только тогда когда его @упомянут (или ответят на его сообщение или вызовут /командой)
```env
...
Чтобы добавить свой MCP инструмент, нужно написать Python функцию и документцию к ней, например в файл   
`mcp_server\app\tools\service.py`
...
`Настройки` -> `Настройки администратора` -> `Интеграции` -> `Управление серверами инструментов` -> добавить (➕) -> выбрать тип `MCP (Streamable HTTP)` -> вписать URL MCP сервера: http://mcp-server:9000/mcp -> установить тип `Вход` на Нет -> вписать любой `ID` и `Имя` -> `Сохранить`  
